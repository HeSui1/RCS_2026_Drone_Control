...
static CANInstance sender_assignment[9] = {
    [0] = {.can_handle = &hfdcan1, .txconf.Identifier = 0x1ff, .txconf.IdType = FDCAN_STANDARD_ID, .txconf.TxFrameType = FDCAN_DATA_FRAME, .txconf.DataLength = 0x08, .txconf.FDFormat = FDCAN_CLASSIC_CAN, .txconf.BitRateSwitch = FDCAN_BRS_OFF, .txconf.ErrorStateIndicator = FDCAN_ESI_ACTIVE, .txconf.TxEventFifoControl = FDCAN_NO_TX_EVENTS, .txconf.MessageMarker = 0, .tx_buff = {0}},
    [1] = {.can_handle = &hfdcan1, .txconf.Identifier = 0x200, .txconf.IdType = FDCAN_STANDARD_ID, .txconf.TxFrameType = FDCAN_DATA_FRAME, .txconf.DataLength = 0x08, .txconf.FDFormat = FDCAN_CLASSIC_CAN, .txconf.BitRateSwitch = FDCAN_BRS_OFF, .txconf.ErrorStateIndicator = FDCAN_ESI_ACTIVE, .txconf.TxEventFifoControl = FDCAN_NO_TX_EVENTS, .txconf.MessageMarker = 0, .tx_buff = {0}},
    [2] = {.can_handle = &hfdcan1, .txconf.Identifier = 0x2ff, .txconf.IdType = FDCAN_STANDARD_ID, .txconf.TxFrameType = FDCAN_DATA_FRAME, .txconf.DataLength = 0x08, .txconf.FDFormat = FDCAN_CLASSIC_CAN, .txconf.BitRateSwitch = FDCAN_BRS_OFF, .txconf.ErrorStateIndicator = FDCAN_ESI_ACTIVE, .txconf.TxEventFifoControl = FDCAN_NO_TX_EVENTS, .txconf.MessageMarker = 0, .tx_buff = {0}},
    [3] = {.can_handle = &hfdcan2, .txconf.Identifier = 0x1ff, .txconf.IdType = FDCAN_STANDARD_ID, .txconf.TxFrameType = FDCAN_DATA_FRAME, .txconf.DataLength = 0x08, .txconf.FDFormat = FDCAN_CLASSIC_CAN, .txconf.BitRateSwitch = FDCAN_BRS_OFF, .txconf.ErrorStateIndicator = FDCAN_ESI_ACTIVE, .txconf.TxEventFifoControl = FDCAN_NO_TX_EVENTS, .txconf.MessageMarker = 0, .tx_buff = {0}},
    [4] = {.can_handle = &hfdcan2, .txconf.Identifier = 0x200, .txconf.IdType = FDCAN_STANDARD_ID, .txconf.TxFrameType = FDCAN_DATA_FRAME, .txconf.DataLength = 0x08, .txconf.FDFormat = FDCAN_CLASSIC_CAN, .txconf.BitRateSwitch = FDCAN_BRS_OFF, .txconf.ErrorStateIndicator = FDCAN_ESI_ACTIVE, .txconf.TxEventFifoControl = FDCAN_NO_TX_EVENTS, .txconf.MessageMarker = 0, .tx_buff = {0}},
    [5] = {.can_handle = &hfdcan2, .txconf.Identifier = 0x2ff, .txconf.IdType = FDCAN_STANDARD_ID, .txconf.TxFrameType = FDCAN_DATA_FRAME, .txconf.DataLength = 0x08, .txconf.FDFormat = FDCAN_CLASSIC_CAN, .txconf.BitRateSwitch = FDCAN_BRS_OFF, .txconf.ErrorStateIndicator = FDCAN_ESI_ACTIVE, .txconf.TxEventFifoControl = FDCAN_NO_TX_EVENTS, .txconf.MessageMarker = 0, .tx_buff = {0}},
    [6] = {.can_handle = &hfdcan3, .txconf.Identifier = 0x1ff, .txconf.IdType = FDCAN_STANDARD_ID, .txconf.TxFrameType = FDCAN_DATA_FRAME, .txconf.DataLength = 0x08, .txconf.FDFormat = FDCAN_CLASSIC_CAN, .txconf.BitRateSwitch = FDCAN_BRS_OFF, .txconf.ErrorStateIndicator = FDCAN_ESI_ACTIVE, .txconf.TxEventFifoControl = FDCAN_NO_TX_EVENTS, .txconf.MessageMarker = 0, .tx_buff = {0}},
    [7] = {.can_handle = &hfdcan3, .txconf.Identifier = 0x200, .txconf.IdType = FDCAN_STANDARD_ID, .txconf.TxFrameType = FDCAN_DATA_FRAME, .txconf.DataLength = 0x08, .txconf.FDFormat = FDCAN_CLASSIC_CAN, .txconf.BitRateSwitch = FDCAN_BRS_OFF, .txconf.ErrorStateIndicator = FDCAN_ESI_ACTIVE, .txconf.TxEventFifoControl = FDCAN_NO_TX_EVENTS, .txconf.MessageMarker = 0, .tx_buff = {0}},
    [8] = {.can_handle = &hfdcan3, .txconf.Identifier = 0x2ff, .txconf.IdType = FDCAN_STANDARD_ID, .txconf.TxFrameType = FDCAN_DATA_FRAME, .txconf.DataLength = 0x08, .txconf.FDFormat = FDCAN_CLASSIC_CAN, .txconf.BitRateSwitch = FDCAN_BRS_OFF, .txconf.ErrorStateIndicator = FDCAN_ESI_ACTIVE, .txconf.TxEventFifoControl = FDCAN_NO_TX_EVENTS, .txconf.MessageMarker = 0, .tx_buff = {0}},
...
//		USART_Vofa_Justfloat_Transmit(measure->speed_aps, RPM_2_ANGLE_PER_SEC*(float)((int16_t)(rxbuff[2] << 8 | rxbuff[3])),0);
...
static void DJIMotorLostCallback(void *motor_ptr)
{
    ...
    uint16_t can_bus = 0;
    if(motor->motor_can_instance->can_handle == &hfdcan1) can_bus = 1;
    else if(motor->motor_can_instance->can_handle == &hfdcan2) can_bus = 2;
    else if(motor->motor_can_instance->can_handle == &hfdcan3) can_bus = 3;
...
void DJIMotorControl()
{
    ...
    int16_t set = 0;        // 电机控制CAN发送设定值
    DJIMotorInstance *motor;
    Motor_Control_Setting_s *motor_setting; // 电机控制参数
    Motor_Controller_s *motor_controller;   // 电机控制器
    DJI_Motor_Measure_s *measure;           // 电机测量值
    float pid_measure, pid_ref = 0;             // 电机PID测量值和设定值
    ...
    { // 减小访存开销,先保存指针引用
        ...
        // 获取最终输出
        set = (int16_t)pid_ref;
        //USART_Vofa_Justfloat_Transmit(pid_ref,pid_measure,motor_controller->pid_ref);
        // 分组填入发送数据
