...
void SBUS_TO_RC(volatile const uint8_t *sbus_buf, Remote_Info_Typedef  *remote_ctrl)
{
    ...
    rc_data.rc.switch_left=remote_ctrl->rc.s[1];
    rc_data.rc.switch_right=remote_ctrl->rc.s[0];
    /* Mouse axis: X, Y, Z */
    ...
    rc_data.mouse.x = remote_ctrl->mouse.x;
    rc_data.mouse.y = remote_ctrl->mouse.y;
    ...
    rc_data.mouse.press_l = remote_ctrl->mouse.press_l;
    rc_data.mouse.press_r = remote_ctrl->mouse.press_r;
    /* KeyBoard value */
    remote_ctrl->key.v = sbus_buf[14] | (sbus_buf[15] << 8);                    //!< KeyBoard value
    rc_data.key[0].keys = remote_ctrl->key.v;
    ...
    rc_data.rc.dial=remote_ctrl->rc.ch[4];
    rc_data.rc.rocker_l1=remote_ctrl->rc.ch[3];
    rc_data.rc.rocker_r1=remote_ctrl->rc.ch[1];
    rc_data.rc.rocker_l_=remote_ctrl->rc.ch[2];
    rc_data.rc.rocker_r_=remote_ctrl->rc.ch[0];
