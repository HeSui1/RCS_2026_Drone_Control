...
//	// 【修改点 1】：获取指针，而不是直接获取结构体的值
//				 Graph_Data_t *graphData = va_arg(ap, Graph_Data_t*); 
//				 
//				 // 【修改点 2】：拷贝时直接用指针，去掉原来的 (uint8_t *)&graphData
//				 memcpy(buffer + (LEN_HEADER + LEN_CMDID + Interactive_Data_LEN_Head + UI_Operate_LEN_PerDraw * i), (uint8_t *)graphData, UI_Operate_LEN_PerDraw);
//		}
...
// === 直接覆盖替换原来的 UIGraphRefresh ===
void UIGraphRefresh(referee_id_t *_id, Graph_Data_t *graph)
{
    UI_GraphReFresh_t UI_GraphReFresh_data;
    
    // 既然只画 1 个，长度全部固定
    uint8_t temp_datalength = LEN_HEADER + LEN_CMDID + Interactive_Data_LEN_Head + UI_Operate_LEN_PerDraw + LEN_TAIL; 
    static uint8_t buffer[64]; // 只需要发 30 个字节，缓存给 64 足矣

    // 1. 组装帧头
    UI_GraphReFresh_data.FrameHeader.SOF = REFEREE_SOF;
    UI_GraphReFresh_data.FrameHeader.DataLength = Interactive_Data_LEN_Head + UI_Operate_LEN_PerDraw;
    UI_GraphReFresh_data.FrameHeader.Seq = UI_Seq;
    UI_GraphReFresh_data.FrameHeader.CRC8 = Get_CRC8_Check_Sum((uint8_t *)&UI_GraphReFresh_data, LEN_CRC8, 0xFF);

    // 2. 组装命令码和数据头
    UI_GraphReFresh_data.CmdID = ID_student_interactive;
    UI_GraphReFresh_data.datahead.data_cmd_id = UI_Data_ID_Draw1; // 强制指定：就是画 1 个图形
    UI_GraphReFresh_data.datahead.receiver_ID = _id->Cilent_ID;
    UI_GraphReFresh_data.datahead.sender_ID = _id->Robot_ID;

    // 3. 内存拷贝：把头部数据复制进 buffer
    memcpy(buffer, (uint8_t *)&UI_GraphReFresh_data, LEN_HEADER + LEN_CMDID + Interactive_Data_LEN_Head);

    // 4. 内存拷贝：直接把你传进来的那个圆的结构体复制进 buffer
    memcpy(buffer + LEN_HEADER + LEN_CMDID + Interactive_Data_LEN_Head, (uint8_t *)graph, UI_Operate_LEN_PerDraw);
